...
class Messages_IndexController extends Rastor_Controller_Action {
	...
	$auth = $this->_getAuth()->getIdentity();
	$messageModel = new Messages_Model_DbTable_Actions();
	$messages = $messageModel->getUserMessagesById($auth->id);
	$this->view->messages = $messages;
	//Zend_Debug::dump($messages);die;
    }

    public function markasreedAction() {
	$this->_helper->layout()->disableLayout();
	$topic_db = new Messages_Model_DbTable_Actions();
	$id = $this->_getParam('mesegeid');
	$topic_db->update(array('flag_reed'=>1), $id);
	$this->_redirect('/actions');
    }
