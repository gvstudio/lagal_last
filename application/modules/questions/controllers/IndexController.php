...
class Questions_IndexController extends Rastor_Controller_Action {
	...
	$topic_db = new Questions_Model_DbTable_TopicRelations();
	$id_user = $this->_getParam('userid', 0);
	$id_topic = $this->_getParam('topicid', 0);
	$action = $this->_getParam('act',2);
	if($action == 1){ //если подписатся то подписываемся 
		$insert_array = array(
		    'user_id' => $id_user,
		    'question_id' => $id_topic
		);
		$topic_db->getAdapter()->insert('topic_relations', $insert_array);
	//}
	}
	if($action == 0){ //отписываемся
	    $where = 'question_id = '. $id_topic ; 
	    $topic_db->getAdapter()->delete('topic_relations', $where);
	}
	$this->_redirect('/');
    }
	...
	// Как поставили коммент возьмем чуваков которые подписаны на тему.
	$subscribers = $questioncomments->getSubscribers($id);
	
	//теперь отошлем сообщеиния чувакам
	$messages = new Messages_Model_DbTable_Actions();
	$messages->addSystemMessages($subscribers);
	//Zend_Debug::dump($subscribers);die;
